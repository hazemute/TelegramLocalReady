...
void ConnectionsManager::initDatacenters() {
    // Self-hosted MyTelegram bootstrap DCs. The server returns its full DC list
    // after the first connection; keeping every bootstrap id on the local host
    // prevents accidental fallback to Telegram's public infrastructure.
    const std::string ipv4 = MyTelegramLocalConfig::kServerIpv4;

    if (datacenters.find(1) == datacenters.end()) {
        auto *datacenter = new Datacenter(instanceNum, 1);
        datacenter->addAddressAndPort(ipv4, MyTelegramLocalConfig::kBootstrapPort, 0, "");
    ...
    if (datacenters.find(3) == datacenters.end()) {
        auto *datacenter = new Datacenter(instanceNum, 3);
        datacenter->addAddressAndPort(ipv4, MyTelegramLocalConfig::kBootstrapPort, 0, "");
